...
The trace-captured-attention, untrimmed-decoder, and browser frontend parity issues are resolved in this feasibility harness. The result remains **NO-GO** until it has end-to-end browser corpus synthesis with seeded noise, WAV/playback, cached-offline assets, renderer latency/memory measurements, numerical audio parity, and listening evidence. The plugin must be distributed GPL-3.0-or-later-compatible with ephone/eSpeak corresponding-source and notices. Do not use a Python sidecar, native ORT, or substitute model as a workaround under this decision.
