...
The GPL-3.0-or-later license of eSpeak-NG blocks its distribution as the required browser frontend under the Apache-compatible constraint. Alternatives were audited: ephone and Piper paths inherit eSpeak-NG; Phonetisaurus.js is MIT but does not provide a licensed English FST model; Misaki/Pyodide is a large non-equivalent Python/WASM stack with component-specific licensing. The browser-targeted pure-JS MIT package `phonemize` 2.0.0 was empirically screened with the exact upstream normalized strings. It achieved **0/6 exact phoneme matches** and only **4.9%–9.4% character-position agreement** across the corpus, with systematic stress, vowel, consonant, and pronunciation differences. Its detailed output is committed in `fixtures/permissive-g2p-evaluation.json`. It is not a validated equivalent and must not feed the model.

## Caveats and required next work

The trace-captured-attention and untrimmed-decoder issues are resolved in this feasibility harness. Under the present license constraint, the browser frontend gate has no viable candidate and the result remains **NO-GO**. A future GO requires either approval to ship eSpeak-NG under GPL-compatible terms, or a new permissively licensed G2P plus data that demonstrates acceptable corpus and audio parity. Then define chunk rejection/splitting behavior for the fixed 512-token core window and collect browser latency/memory/offline data, numerical audio parity on real corpus tokens, and blind listening results. Do not use a Python sidecar, native ORT, or substitute model as a workaround under this decision.
