...
for (const prompt of prompts.filter((entry) => entry.id !== "long")) {
  ...
  try {
    ...
    throw new Error(`could not parse browser result for ${prompt.id}`, { cause: error });
  }
}
writeFileSync("fixtures/browser-corpus-results.json", `${JSON.stringify(results, null, 2)}\n`);
console.log(`BROWSER_CORPUS_OK cases=${results.length}`);
console.log("BROWSER_CORPUS_LONG_TEXT_DEFERRED requires chunk concatenation runtime");
