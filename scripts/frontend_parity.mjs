...
try {
  ...
  await page.goto(`http://127.0.0.1:${server.address().port}/browser/index.html?parity=1`, { waitUntil: "networkidle", timeout: 120000 });
  const result = JSON.parse(await page.locator("body").textContent());
  if (!result.ok) throw new Error(`FRONTEND_PARITY_FAILED ${JSON.stringify(result)}`);
